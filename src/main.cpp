...
const string strMessageMagic = "Cypherfunk Signed Message:\n";
...
{
    // Cypherfunk: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 60 * 60 * 4; // Cypherfunk: 4 hours
static const int64 nTargetSpacing = 120; // Cypherfunk: 2 minutes
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Cypherfunk: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Cypherfunk: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        //+1 from mainnet
        pchMessageStart_new[0] = 0xfd;
        pchMessageStart_new[1] = 0xc4;
        pchMessageStart_new[2] = 0xbb;
        pchMessageStart_new[3] = 0xe3;
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CypherfunkMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CypherfunkMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CypherfunkMiner(CWallet *pwallet)
{
    printf("CypherfunkMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("Cypherfunk-miner");
    ...
    try { loop {
        ...
        printf("Running CypherfunkMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CypherfunkMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CypherfunkMiner, pwallet));
}
