...
#if defined(NDEBUG)
# error "Cypherfunk cannot be compiled without assertions."
#endif
...
const std::string strMessageMagic = "Cypherfunk Signed Message:\n";
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        {
            LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s FUNK additional fees, %d delta bytes\n",
...
static bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex)
{
    ...
    if (hash != chainparams.GetConsensus().hashGenesisBlock) {
        ...
        if (mi == mapBlockIndex.end())
            return state.DoS(100, error("%s: prev block not found", __func__), 0, "prev-blk-not-found"); 
            //the above line was state.DoS(10,...
            //we block all peers giving us bad blocks, because the old client's behavior meant connecting to anyone
            //including litecoin. we don't want their blocks, nor do we want them on our peers
            //so we block them. a later version of this client software can reduce it back to 10
            //only connecting to peers with version 70036 (this client's version).. see version.h
